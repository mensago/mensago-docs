...
Each of the fields defined in a contact are described below. Unless otherwise indicated, each field is assigned to the privacy group `private`.
...
Mensago:Keys:: CONDITIONAL. Privacy level `secret`. This field group sublist contains the contact’s Mensago encryption keys. Each key is named by its purpose. These are currently `signing`, `primary`, `social`, or `broadcast`. It is a required part of the `mensago` field group.
