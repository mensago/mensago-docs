...
Public:: The least private information group. By default the following fields are set to this level: `FormattedName`, `GivenName`, `FamilyName`, `Gender`, `Prefixes`, `Suffixes`, and all `Mensago` fields except `Keys`. If a Mensago client implements introductions, only information at this level may be shared with others.

Private:: Most contact information fields are found in this privacy group by default. If a contact request is sent including this level of information, information from the Public level is also included, but information from this level overrides any field in the Public group.

Secret:: The most private of the groups. If a contact request is sent including this level of information, information from the other levels is also included, but information from this level overrides any field found in either of them. Mensago cryptography keys are in this group by default.
...
Header:Update:: REQUIRED. Describes whether or not the information in the contact is intended to update existing information. This payload field is present only in contact information update messages.
...
Mensago:Keys:: CONDITIONAL. Privacy level `secret`. This field group sublist contains the contact’s Mensago encryption keys. Each key is named by its purpose. These are currently `verify`, `encrypt`, `social`, or `broadcast`. It is a required part of the `mensago` field group.
