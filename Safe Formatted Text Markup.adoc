= Safe Formatted Text Markup (SFTM) and the Safe Document Format (SDF)
:author: Jon Yoder
...
* _Unnecessary Complexity_ - No CSS. Simpler syntax. Easier parsing. Whitespace characters in the document body -- except inside tags -- are preserved.
...
As seen in the list above, formatting for an escaped character is the prefix `\u` followed by the decimal value of the character and a semicolon. Although escape codes could be used for other characters, these are the only ones permitted at this time; it is better to just use the character itself for visual clarity and to prevent obfuscation.
...
This tag is used to enclose all text in the document. Any text preceding the opening tag or following the closing tag MUST be ignored or, even better, removed. 
...
- *version*: OPTIONAL. Indicates the version of the standard used. Until the specification reaches final approval this will always be `1.0`.

- *doctype*: OPTIONAL. Indicates the type of document. It MUST be either `sftm` or `sdf` and dictates how the document is to be interpreted. For example, an SDF document with a doctype of `sftm` will cause the parser and/or interpreter to ignore SDF tags and extended attributes. Likewise, an SFTM document with a doctype of `sdf` will cause it to be interpreted as an SDF document with any defaults filling in the missing information.  
...
----
[head]
[linkinfo name="link1" site:"youtube.com" ]Skillet - Hero (Official Video)[/linkinfo]
[/head]
----
...
The `linkinfo` tag provides additional information about the target of a link which appears in the body of the document. Its intent is for client software to automatically obtain contextual information for link to be displayed so that a user can make an informed decision to click or not click on it. The title of the page linked to appears within the tag pair. This title should taken from the header of the page and MUST not be editable by the user. If the link target does not specify a title or if it is a downloadable file, the title may be empty.
...
----
[head]
[fontinfo name="Roboto" fallback="Helvetica, sans-serif"][/fontinfo]
[/head]
----
...
The `fontinfo` tag specifies information about a nonstandard font used in the document. The `fallback` parameter is a comma-separated list of other font families which could be used instead of the one specified. The final entry in this list should be a generic type, such as `mono`,`serif`, or `sans-serif`. Text inside the tag pair is currently ignored.
...
== anchor:sdf-crypto[]SDF Digital Signatures and Encryption

Building upon the foundations of the Mensago communications platform, SDF documents can also be signed. Two other sections may exist beyond the document's ending `[/document]` tag, one for a digital signature and one for encryption.

=== anchor:signature[]Signature

*Syntax*

 [signature type="ed25519" encoding="base85" address="csimons/example.com"]vp6R*W==)9>Us`MA!pz>7yyT&2{I&;`8hQmP<I2uJqu*<OA`O}!a^*e!W5dkVJ%_~V>KQpl;wN+#kXe)[/signature]

*Parameters*

- *type*: The signing algorithm used. Currently the only supported algorithms are `ed25519`, `rsa2048`, and `rsa4096`. 

- *encoding*: The type of binary-to-text algorithm used, which may either be `base64` or `base85`.

- *address*: Address of the entity signing the document. This may be a regular Mensago address or a workspace address.

*Notes*

This tag, if it exists, MUST appear immediately following the document's closing `[/document]` tag. The text enclosed by the tag pair is the encoded digital signature of the entire document, including the `[document]` tag pair. This tag pair is required if encryption is used, in which case document contents are signed before they are encrypted.

=== anchor:docencryption[]Encryption

*Syntax*

 [encryption type="xsalsa20" keyencoding="base85" keyencryption="curve25519" docencoding="base85"]ihbCfWuF8`7x}bk7vBjJel?p52RB8ALs3MNy)CSw[/encryption]

*Parameters*

- *type*: The encryption algorithm used. Currently the only supported algorithms are `aes128`, `aes256`, or `xsalsa20`.

- *keyencoding*: The type of binary-to-text algorithm used on the encryption key, which may either be `base64` or `base85`.

- *keyencryption*: CONDITIONAL. The type of encryption used to encrypt the document key, which may be `curve25519`, `rsa2048`, or `rsa4096`. This parameter is required if the document key is encrypted with a public key. If password-based encryption is utilized, this parameter is not used.

- *docencoding*: The type of binary-to-text algorithm used to encode the encrypted document contents, which may either be `base64` or `base85`.

*Notes*

This tag, if it exists, MUST appear immediately following the document's closing `[/signature]` tag. The contents of the document are encrypted, encoded using either Base64 or Base85 encoding, and placed between the two `document` tags.
