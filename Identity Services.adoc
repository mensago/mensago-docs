...
Securing a domain’s resource records with DNSSEC cannot be recommended enough. When validated DNSSEC signatures are present, a facility similar to https://en.wikipedia.org/wiki/DNS-based_Authentication_of_Named_Entities[DANE] may be used to validate the domain’s TLS certificate. In such situations, a certificate signed by a recognized third-party certificate authority (CA) is not required. In light of CA compromises in recent years, this is a notable benefit. Without the presence of DNSSEC, resolvers MUST require the TLS certificate of a Mensago server to be signed by a recognized third party CA.
...
. Perform an A or AAAA lookup for `mensago.subdomain.example.com`
. Perform an A or AAAA lookup for `mensago.example.com`
...
ek:: REQUIRED. This contains the CryptoString-formatted encryption key corresponding to the `Encryption-Key` found in the organization’s current keycard entry. This key is used for clients to authenticate servers. 
...
Encryption-Key:: a public key in CryptoString format for third party use. Possible uses include website authentication or PGP. Although reserved, this field is required.

Verification-Key:: a verification key in CryptoString format for third party use. Possible uses include signing documents and other files.
