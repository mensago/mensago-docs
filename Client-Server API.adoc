...
There are five types of updates: CREATE, DELETE, MOVE, MKDIR, and RMDIR.
...
`CREATE` and `DELETE` updates list the full path of the new item and is received even if the item is not part of the selected directory. They also operate only on files.`MKDIR` and `RMDIR` are specific to directories. `RMDIR` is only used to remove empty directories. In order to delete a directory and its contents, all files must be `DELETE`ed before the directory can be removed with `RMDIR`. `MOVE` updates follow the format of the MOVE command, providing the full path of the item prior to the move and then the new directory to which it was moved. Both paths are returned in the same string, in this case.
