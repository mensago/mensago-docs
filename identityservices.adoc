...
Paranoid Mode is feature not technically a requireed for Anselus clients to implement, but is encouraged. Simply put, it is a set of security settings for those users who are particularly concerned about their data security. It is designed to quickly configure a user's client for maximum security and privacy. It consists of the following settings:

- Restricting access to only one device, reducing server-side data storage to messages waiting for delivery.
- Preventing the use of a User ID or Name in a keycard

TODO: Finish Paranoid Mode
