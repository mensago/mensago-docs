...
The problems which MenTM is intended to solve in replacing HTML for rich formatting are as follows:
...
MenTM marks text with pairs of tags enclosed in square brackets. Each opening tag MUST have a corresponding closing tag. Each closing tag MUST begin with a forward slash (/) immediately before the text of the
...
Although it is expected that most content will conform to this specification, there will be instances where it does not. This section defines some general rules for handling incorrect content. 
...
Unlike HTML, tag pairs are used with images in MenTM documents, with the enclosed text to be rendered as the image's caption. A number by itself MUST be considered a pixel size and a number ending in a percent symbol MUST be interpreted as a percentage relative to the image's size in pixels.
...
MenTM color references follow Web standards, supporting 140 different color names and associated hextet values.
