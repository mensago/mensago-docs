...
The Remaining field exists for circumstances where there are an unusually large number of updates requested. The field contains the number of updates not returned by the server. Server implementors would be wise to set a maximum number of updates returned by one transaction.
