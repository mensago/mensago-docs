...
_Gets the disk quota for the current workspace. Administrators may request the quota for other workspaces._

[cols="1,3a"]
|===
| Parameters | * Workspace (administrator only)
| Returns | * 200 OK
** Size
...
GETQUOTA obtains the disk quota value for the current workspace. An administrator account can also include a workspace ID in the Workspace parameter to obtain the disk quota for another workspace. The Size value returned is the disk quota measured in mebibytes. If issued the Workspace parameter is included in the command when issued by a non-administrator, `403 FORBIDDEN` is returned.
...
SETQUOTA customizes a workspace's disk quota size. The Size parameter is measured in mebibytes and is expected to be an integer greater than 0. The Workspaces parameter is a string containing a comma-separated list of workspace IDs. If issued by a non-administrator, `403 FORBIDDEN` is returned.
